...
- [ ] **PR #9909 en espera de Glama**: el bot exige listing en Glama + badge de score. Los 4 servers ya submitted (12 jul, en revisión de Glama) con Dockerfiles en `mcp/Dockerfile`. Cuando las páginas estén públicas (patrón `glama.ai/mcp/servers/@csacanam/<repo>` o sin @): agregar los 4 badges tras la descripción de cada entrada en la rama `add-csacanam-servers` del fork y pushear. Formato: `[![csacanam/comprabtc MCP server](https://glama.ai/mcp/servers/<ruta>/badges/score.svg)](https://glama.ai/mcp/servers/<ruta>)`
